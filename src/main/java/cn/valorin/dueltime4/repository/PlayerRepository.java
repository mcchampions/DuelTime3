...
public class PlayerRepository {
    ...
    public void createTableIfNotExists() {
        db.executeDDL("CREATE TABLE IF NOT EXISTS player_data ("
    ...
    public void upsert(PlayerProfile profile) {
        try (SqlHelper sql = db.open()) {
            String[] cols = {"exp", "point", "classic_wins", "classic_loses", "classic_draws",
                "total_games", "total_time", "win_streak", "max_win_streak"};
            sql.update("INSERT INTO player_data (player_name, exp, point, classic_wins, classic_loses, "
                + "classic_draws, total_games, total_time, win_streak, max_win_streak) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
                + db.upsertSql("player_name", cols),
